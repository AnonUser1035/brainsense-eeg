...
function [y, fs] = playStereoTones_sleep(f_carry, f_assr, f_env, dur, fs, m, ramp_ms, level_dBFS, doPlay)
...
% -------- Long fade in/out: 1/3 – 1/3 – 1/3 --------
fadeN = floor(N/3);        % samples for fade in and fade out
steadyN = N - 2*fadeN;    % middle section
...
% Steady section
steady = ones(1, steadyN);

% Full window
win = [fadeIn, steady, fadeOut];

% Safety: make sure length matches exactly
win = win(1:N);
