...
unpackedFile.channelsData     = channelsData(:,1:end-6);
unpackedFile.PID              = PIDs(:,:);
unpackedFile.packetTimestamp  = packetTS(:,:);      % raw uint32 timestamp from each packet footer
if max(packetTimeSec(:))==0 | isnan(packetTimeSec(:))
    packetTimeSec = (0:(length(channelsData(1,:))-1))./unpackedFile.channelSampleRate;
end
unpackedFile.time    = packetTimeSec(:,1:end-6); % packet start times computed from sampleRate (seconds)
